    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        switch (method){
            case "login":
               ...
               //不区分大小写
                ...
                try {
                    ...
                    hexStr = ByteUtils.toHexAscii(digest);
                } catch (Exception e) {
                    e.printStackTrace();
                }
                ...
                User user = userBiz.getUser(name,hexStr);
            ...
            case "exit":
                ...
                //1.清除session
                session.invalidate();
                //2.跳转到login.html(框架中需要回去)  top.jsp->parent->index.jsp 整个页面进行跳转
            ...
            case "modifyPwd":
                ...
                try {
                    ...
                    MessageDigest md5 = MessageDigest.getInstance("MD5");
                    // 转换为MD5码
                    byte[] digest = md5.digest(newPwd.getBytes("utf-8"));
                    hexStr2 = ByteUtils.toHexAscii(digest);
                } catch (Exception e) {
                    e.printStackTrace();
                }

                //3.调用biz层方法
                int count = userBiz.modifyPwd(id,hexStr2);
                //4.响应-参考exit
                if(count>0){
...
//                }
                ...
                //修改密码
                //1.获取用户输入的新的密码
                String newPwd2 = req.getParameter("newpwd");
                ...
                try {
                    ...
                    MessageDigest md5 = MessageDigest.getInstance("MD5");
                    // 转换为MD5码
                    byte[] digest = md5.digest(newPwd2.getBytes("utf-8"));
                    hexStr3 = ByteUtils.toHexAscii(digest);
                } catch (Exception e) {
                    e.printStackTrace();
                }

                //3.调用biz层方法
                int count1 = userBiz.add(userName,hexStr3);
                //4.响应-参考exit
                if(count1>0){
                    out.println("<script>alert('密码修改成功');parent.window.location.href='login.html';</script>");
                }else{
