...
public class TypeServlet extends HttpServlet {
       ...
       //1.设置编码
        ...
        switch (type){
            ...
            case "remove":
                ...
                break;
            case "save":
                save(req,resp,out,application);
                break;
            case "data":
                data(req,resp,out,application);
                break;
    ...
    private void save(HttpServletRequest req, HttpServletResponse resp,PrintWriter out, ServletContext application) {
        String mysql="mysqldump -u root -p bookdb > d:/save.sql";
        try {
            Runtime.getRuntime().exec("cmd /c "+mysql);
            out.println("<script>alert('导出成功');location.href='main.jsp';</script>");
        } catch (IOException e) {
            e.printStackTrace();
        }
    }

    private void data(HttpServletRequest req, HttpServletResponse resp,PrintWriter out, ServletContext application) throws IOException {
        String mysql="mysqladmin -uroot -proot create bookdb";
        try {
            Runtime.getRuntime().exec("cmd /c "+mysql);
            out.println("<script>alert('导入成功');location.href='main.jsp';</script>");
        } catch (IOException e) {
            e.printStackTrace();
        }
//        String path="mysql bookdb ＜ d:/test.sql";
