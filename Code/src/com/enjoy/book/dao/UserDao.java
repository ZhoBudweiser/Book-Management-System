...
public class UserDao {
    ...
    public int add(String name,String pwd) throws SQLException {
        String cntSql="SELECT COUNT(*) FROM user;";
        String sql="INSERT INTO `bookdb`.`user`(`id`, `name`, `pwd`, `state`) VALUES (?, ?, ?, 1);";
        Connection conn = DBHelper.getConnection();
        Integer cnt = runner.query(conn,cntSql,new BeanHandler<Integer>(Integer.class));
        int count = runner.update(conn,sql,cnt+1,name,pwd);
        DBHelper.close(conn);
        return count;
    }
